...
local anim = {
  fn = {
    SPIKE = {fn = function(x) return 2^(5*x - 5) end, fn_init = 0, fn_end = 1.2},
...
local function lerp2(x, a, b, af, bf)  -- linear interpolation
  return af + ((x - a) / (b - a)) * (bf - af)
end

local function clamp(x, min, max) -- TAKEN FROM LUME!!!
...
local function frame_eq(x, init, fin, last_frame)
  return clamp(
    math.ceil(lerp2(x, init, fin, 1, last_frame)), 1, last_frame
  )
end

local function frame_eq_free(x, init, fin, last_frame)
  return math.ceil(
    lerp2(x, init, fin, 1, last_frame)
  )
end
...
function anim:move(params) -- {[id,] obj, to, [duration, fn]}
